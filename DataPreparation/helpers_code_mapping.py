...
class CodeMappingHelper:
            ...
            if 'titleWindow' in step['dynGuiComponent']:
                ...
                if len(comp_window)>0:
                    ...
                    for str_id in strings_ids:
                        ...
                        rch_terms.extend(android_ids)
                    rch_terms.extend(strings_ids)
    ...
    def get_additional_rch_terms_from_screen_procecessed(self, gui_components):
        additional_rch_terms = []
            ...
            if 'text' in component:
                if len(component['text'])>0:
                    ...
                    for each_text in comp_text:
                        if len(each_text)>0:
                            additional_rch_terms.append(each_text)
            if 'idXml' in component:
                if len(component['idXml'])>0:
                    ...
                    for each_id in comp_id: 
                        if len(each_id)>0:
                            additional_rch_terms.append(each_id)
        additional_rch_terms = self.get_unique_names(additional_rch_terms)

        return additional_rch_terms
        ...
        for component in gui_components:
            if 'idXml' in component:
                if len(component['idXml'])>0:
                    ...
                    comp_id = component_idxml[len(component_idxml)-1]
                    if len(comp_id)>0:
                        additional_rch_terms.append(comp_id)
        additional_rch_terms = self.get_unique_names(additional_rch_terms)
        return additional_rch_terms
    ...
    def remove_keywords_from_query(self, rch_terms):
        ...
        for item in rch_terms:
            ...
            if unimportant_word_flag==False:
               modified_rch_terms.append(item)
        return modified_rch_terms
